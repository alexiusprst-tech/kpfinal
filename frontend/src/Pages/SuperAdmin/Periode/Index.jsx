...
export default function PeriodeIndex({ list, stats, tahunAjaranAll, tahunAjaranActive, filters, selectedPeriode }) {
    ...
    const [form, setForm] = useState({
        tahun_ajaran_id: tahunAjaranActive[0]?.id || '', nama: '', tanggal_mulai: today,
        tanggal_selesai: today, deadline_upload: today, catatan: '',
    });
    ...
    const FormFields = ({ onSubmit }) => (
        <form onSubmit={onSubmit} className="space-y-4">
            {!editItem && (
                <div>
                    <label className="block text-xs font-semibold text-gray-700 mb-1">Tahun Ajaran <span className="text-red-500">*</span></label>
                    <select value={form.tahun_ajaran_id} onChange={e => setForm(f => ({ ...f, tahun_ajaran_id: e.target.value }))}
                        ...
                        {tahunAjaranActive.map(ta => <option key={ta.id} value={ta.id}>{ta.nama}</option>)}
                    </select>
                </div>
            )}
            <div>
                                                        ...
                                                        {openMenuId === item.id && (
                                                            <>
                                                                ...
                                                                <div className="absolute right-0 top-9 z-20 bg-white rounded-xl shadow-xl border border-gray-100 py-1.5 w-44">
                                                                    ...
                                                                    {item.status !== 'ACTIVE' && (
                                                                        <>
                                                                            <button onClick={() => openEdit(item)} className="w-full flex items-center gap-2 px-3.5 py-2 text-xs font-semibold text-amber-600 hover:bg-amber-50">
