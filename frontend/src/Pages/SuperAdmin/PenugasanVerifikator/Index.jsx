...
import { Plus, Trash2, AlertTriangle, X, Shield } from 'lucide-react';
...
export default function PenugasanVerifikatorIndex({ list, dosenAll, mkAll, periodeAll, filters }) {
    ...
    const handleFilter = (pid) => {
        ...
        router.get('/superadmin/penugasan-verifikator', { periode_id: pid }, { preserveState: true });
    };

    const handleAdd = (e) => {
        e.preventDefault(); setProcessing(true);
        router.post('/superadmin/penugasan-verifikator', form, { onFinish: () => { setProcessing(false); setShowAdd(false); } });
    };

    const handleDelete = () => {
        setProcessing(true);
        router.delete(`/superadmin/penugasan-verifikator/${deleteItem.id}`, { onFinish: () => { setProcessing(false); setDeleteItem(null); } });
    };
    ...
    return (
        <AuthenticatedLayout title="Penugasan Verifikator">
            <Head title="Penugasan Verifikator" />
            <Toast flash={flash} />
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-extrabold text-gray-800 flex items-center gap-2">
                            <Shield className="w-6 h-6 text-[#801720]" /> Penugasan Verifikator
                        </h1>
                        <p className="text-sm text-gray-500 mt-0.5">Tugaskan dosen sebagai verifikator soal per mata kuliah</p>
                    </div>
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-gray-50 border-b border-gray-100">
                                <tr>
                                    ...
                                    <th className="text-left px-5 py-3.5 text-xs font-bold text-gray-500 uppercase">Verifikator</th>
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {list.data?.length === 0 ? (
                                    <tr><td colSpan={6} className="text-center py-12 text-gray-400">Belum ada penugasan verifikator</td></tr>
                                    ...
                                    <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-5 py-4">
                                            ...
                                            <p className="text-[10px] text-gray-400">{item.dosen?.kode_dosen}</p>
                                        </td>
                                        <td className="px-5 py-4">
                                        ...
                                        <td className="px-5 py-4 text-right">
                                            <button onClick={() => setDeleteItem(item)} className="p-1.5 hover:bg-red-50 text-red-500 rounded-lg">
            ...
            <Modal open={showAdd} onClose={() => setShowAdd(false)} title="Tambah Penugasan Verifikator">
                <form onSubmit={handleAdd} className="space-y-4">
                    <div>
                        <label className="block text-xs font-semibold text-gray-700 mb-1">Dosen Verifikator <span className="text-red-500">*</span></label>
            ...
            <Modal open={!!deleteItem} onClose={() => setDeleteItem(null)} title="Akhiri Penugasan">
                <div className="flex items-start gap-3 mb-5">
                    ...
                    <div>
                        <p className="text-sm font-semibold text-gray-800">Akhiri penugasan <span className="text-[#801720]">{deleteItem?.dosen?.nama_lengkap || deleteItem?.dosen?.nama}</span> sebagai verifikator?</p>
                        <p className="text-xs text-gray-500 mt-1">Status penugasan akan menjadi ENDED.</p>
                    </div>
                </div>
